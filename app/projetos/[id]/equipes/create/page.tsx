...
import { Users, CheckCircle, Loader2, HelpCircle, ArrowLeft, Sparkles, Clock } from "lucide-react"
...
export default function CreateEquipePage() {
  ...
  useEffect(() => {
    const fetchProjeto = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:4000/projetos/${projetoId}`)
        if (!response.ok) {
  ...
  const handleInputChange = (field: keyof CreateEquipeDto, value: string | number) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }))
  ...
  const handleSliderChange = (field: keyof CreateEquipeDto, value: number[]) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value[0],
    }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/projetos/${projetoId}/equipes?success=equipe-criada`)
    } catch (err) {
      setError(err instanceof Error ? err.message : "Erro desconhecido")
    } finally {
      setLoading(false)
  ...
  if (loadingProjeto) {
    return (
      <Layout title="Nova Equipe - EstimAÍ" subtitle="Carregando informações do projeto">
  ...
  return (
    <Layout
      title={`Nova Equipe - ${projeto?.titulo || "Projeto"} - EstimAÍ`}
      subtitle={`Crie uma nova equipe para o projeto ${projeto?.titulo || ""}`}
    >
              ...
              <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
                <CardHeader className="bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-t-lg">
                  ...
                  <CardDescription className="text-blue-100">
                    Informações básicas sobre a equipe para o projeto {projeto?.titulo}
                  </CardDescription>
                </CardHeader>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      {dadosEquipe.map((item) => (
                        <div key={item.key} className="space-y-3">
                          ...
                          <div className="space-y-2">
                            <Slider
                              value={[formData[item.key as keyof CreateEquipeDto] as number]}
                              onValueChange={(value) => handleSliderChange(item.key as keyof CreateEquipeDto, value)}
                              max={5}
                              min={0}
                              step={1}
                              className="w-full"
                            ...
                            <div className="flex justify-between items-center text-xs">
                              {[0, 1, 2, 3, 4, 5].map((num) => (
                                <span
                                  key={num}
                                  className={
                                    formData[item.key as keyof CreateEquipeDto] === num
