...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("userName", userData.name || userData.email) // Save name, fallback to email
      localStorage.setItem("isLoggedIn", "true")

      // Armazenar equipeId se existir (pode ser string ou null)
      if (userData.equipeId) {
        localStorage.setItem("equipeId", userData.equipeId)
      } else {
        localStorage.removeItem("equipeId") // Remove se não existir
      }

      // Redirecionar baseado no tipo de usuário
      if (userData.equipeId) {
        // Usuário é profissional de uma equipe - vai para planning poker
        router.push("/planning-poker")
      } else {
        // Usuário é administrador - vai para histórias
